...
const Record = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=absconder_list&page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
        }
      );
  ...
  const submithandler = async (e) => {
    ...
    formData.append("tag", "absconder_list");
  ...
  return (
    <ProtectedAdminRoute>
      <>
        <Head>
          <title>Absconder List | Admin Dashboard</title>
        </Head>
