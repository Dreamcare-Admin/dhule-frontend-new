...
const Record = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=info_for_police&page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
        }
      );
  ...
  const submithandler = async (e) => {
    ...
    formData.append("tag", "info_for_police");
  ...
  return (
    <ProtectedAdminRoute>
      <>
        <Head>
          <title>Information For Police Officers & Men | Admin Dashboard</title>
        </Head>
