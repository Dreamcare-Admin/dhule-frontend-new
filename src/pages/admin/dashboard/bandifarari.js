...
const Record = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=bandifarari&page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
        }
      );
  ...
  const submithandler = async (e) => {
    ...
    formData.append("tag", "bandifarari");
  ...
  return (
    <ProtectedAdminRoute>
      <>
        <Head>
          <title>Bandifarari | Admin Dashboard</title>
        </Head>
