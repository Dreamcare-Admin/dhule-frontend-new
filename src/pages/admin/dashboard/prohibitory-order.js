...
const Record = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=prohibitary_order&page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
        }
      );
  ...
  const submithandler = async (e) => {
    ...
    formData.append("tag", "prohibitary_order");
  ...
  return (
    <ProtectedAdminRoute>
      <>
        <Head>
          <title>Prohibitory Order | Admin Dashboard</title>
        </Head>
