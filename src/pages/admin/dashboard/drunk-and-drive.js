...
const Record = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=drunk_and_drive_cases&page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
        }
      );
  ...
  const submithandler = async (e) => {
    ...
    formData.append("tag", "drunk_and_drive_cases");
  ...
  return (
    <ProtectedAdminRoute>
      <>
        <Head>
          <title>Drunk and Drive Cases | Admin Dashboard</title>
        </Head>
