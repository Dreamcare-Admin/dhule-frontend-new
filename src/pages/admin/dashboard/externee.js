...
const Record = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=externee&page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
        }
      );
  ...
  return (
    <ProtectedAdminRoute>
      <>
        <Head>
          <title>Externee | Admin Dashboard</title>
        </Head>
