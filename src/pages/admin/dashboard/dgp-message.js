...
const DGPMessage = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-dgp-message?tag=dgp`,
        {
          method: "GET",
        }
      );
  ...
  const submithandler = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/update-dgp-message?Id=${idtemp}`,
        {
          method: "PUT",
          body: formData,
          headers: headers,
          ...
          <div className="flex flex-row">
            ...
            <div className="w-full lg:w-4/5 mt-20 mx-5 bg-white shadow p-5">
              <div className="mb-52 w-full bg-white">
                <h1 className="font-semibold text-xl mt-10 mb-5">DGP Message</h1>
