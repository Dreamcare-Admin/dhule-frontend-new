...
const SeniorOfficers = () => {
  ...
  const fetchADG = async () => {
    try {
      ...
      if (data.Data) {
        const [firstRecord, ...restRecords] = data.Data;
        setRecords1([firstRecord]); // Set the first record in records1 array
        setRecords2(restRecords); // Set the rest of the records in records2 array
      } else {
        //   notifyWarn();
      }
    } catch (error) {
  ...
  return (
    <>
      <Head>
        ...
        <title>Senior Officers | Dhule Police</title>
      </Head>
      ...
      <main className="min-h-screen">
        ...
        <h2 className="text-center font-semibold text-2xl text-gray-700 pt-10 px-5">
          {t("senior")}{" "}
          <span className="text-orange-500">{t("police_officer")}</span>
        </h2>
        <div className="w-28 mt-5 mb-5 border-b border-orange-400 mx-auto"></div>

        <div className="my-10 grid grid-cols-1 gap-5 max-w-4xl mx-auto text-gray-600 justify-items-center">
          {records1.map((record) => {
            return (
              <div
