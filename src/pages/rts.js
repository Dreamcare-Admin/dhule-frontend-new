  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=rts&page=${currentPage}`,
        {
          method: "GET",
        }
      );
  ...
  return (
    <>
      <Head>
        ...
        <title>Right to Service | Dhule Police</title>
      </Head>
      <main className="min-h-screen">
        <Navbar />

        <h1 className="text-center text-4xl text-gray-700 mt-10">{t("rts")}</h1>
