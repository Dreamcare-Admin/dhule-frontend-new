...
const Record = () => {
  ...
  const fetchrecords = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/records-by-tag?tag=nagrikanchi_sanad&page=${currentPage}&limit=${limit}`,
        {
          method: "GET",
        }
      );
  ...
  const submithandler = async (e) => {
    ...
    formData.append("tag", "nagrikanchi_sanad");
  ...
  return (
    <ProtectedAdminRoute>
      <>
        <Head>
          <title>Nagriknachi Sanar | Admin Dashboard</title>
        </Head>
